...
export class Task2Component implements OnInit,AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.Number_of_ball=this.getRandomInt(1,10); // this.getRandomInt(1,10);
  ...
  for (let i = 0; i < this.circles.length; i++) {
    ...
    this.c.fill();
        ...
        if ((this.circles[i].x + this.circles[i].vx + this.circles[i].r >
           this.container.x + this.container.width) || (this.circles[i].x - this.circles[i].r + this.circles[i].vx < this.container.x)) {
          this.circles[i].vx = - this.circles[i].vx;
        }
        if ((this.circles[i].y + this.circles[i].vy + this.circles[i].r >
         this.container.y + this.container.height) || (this.circles[i].y - this.circles[i].r + this.circles[i].vy < this.container.y)) {
        ...
        if (this.circles[i].x + this.circles[i].r >=
           this.canvas.width / 2 - this.s / 2 && this.circles[i].x - this.circles[i].r <= this.canvas.width / 2 + this.s / 2
          && this.circles[i].y + this.circles[i].r >=
           this.canvas.height / 2 - this.s / 2 && this.circles[i].y - this.circles[i].r <= this.canvas.height / 2 + this.s / 2){
                // delete circles[i];
            this.circles[i].r = 0;
        ...
        for (let j = 0; j < this.circles.length; j++) {
          if (j !== i) {
            if (this.check_balls_collision([this.circles[i]],this.circles[j])) {
              const vel_x1:number = this.circles[i].vx;
              const vel_y1:number = this.circles[i].vy;
